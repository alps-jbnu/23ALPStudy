#include <bits/stdc++.h>
using namespace std;

stack<pair<int, int>> s;//디버깅을 위해 전역변수로 뺌
long int cnt = 0;//디버깅을 위해 전역변수로 뺌

void debug(int num) {
	cout << "----debug" << num << "----\n";
	if (s.empty()) {
		cout << "현재 스택은 비어있습니다\n";
		return;
	}
	stack<pair<int, int>> sub_s;
	while (!s.empty()) {
		cout << '<' << s.top().first << ',' << s.top().second << ">\n";
		sub_s.push({ s.top().first ,s.top().second });
		s.pop();
	}
	while (!sub_s.empty()) {
		s.push({ sub_s.top().first ,sub_s.top().second });
		sub_s.pop();
	}
	cout << "cnt: "<< cnt << '\n';
}

int main() {
	int n, input;
	cin >> n;
	cin >> input;
	s.push({ input,1 });
	n--;
	cout << "new input: " << input << '\n';
	debug(1);
	while (n--) {
		cin >> input;
		//debug
		cout << "new input: " << input << '\n';
		if (input < s.top().first) {
			//내림차순: 계속 입력받는다
			s.push({ input,1 });
			debug(2);
		}
		else if (input == s.top().first) {
			//같은 수: 같은 수 끼리 중첩한다 =>top스택의 second에 ++
			int mem = s.top().second;
			s.pop();
			s.push({ input,mem + 1 });
			debug(3);
		}
		else if (input > s.top().first) {
			//오름차순: 제거 연산 진행, 마지막 스택에 대한 예외처리 필요
			while (!s.empty()) {//input보다 큰 수가 나오기 전까지 제거
				if (input > s.top().first) {
					if (s.top().second > 1) {//중첩된 항목인지 검사
						int n = s.top().second;
						s.pop();
						if (s.size() < 1) {
							cnt = cnt + n + n * (n - 1) / 2;
						}
						else {
							cnt = cnt + 2 * n + n * (n - 1) / 2;
						}
						debug(4);
					}//중첩되지 않았다면
					else {
						s.pop();
						if (s.size() < 1) {
							cnt += 1;
						}
						else {
							cnt += 2;
						}
						debug(5);
					}
				}
				else {
					break;
				}
			}
			if (s.empty()) {
				s.push({ input,1 });
				debug(6);
			}
			else {
				if (input == s.top().first) {
					//같은 수: 같은 수 끼리 중첩한다 =>top스택의 second에 ++
					int mem = s.top().second;
					s.pop();
					s.push({ input,mem + 1 });
					debug(7);
				}
				else {
					s.push({ input,1 });
					debug(8);
				}
			}
		}
	}
	while (!s.empty()) {
		if (s.top().second > 1) {//중첩된 항목인지 검사
			int n = s.top().second;
			s.pop();
			if (s.size() < 1) {
				cnt = cnt + n * (n - 1) / 2;
			}
			else {
				cnt = cnt + n + n * (n - 1) / 2;
			}
			debug(9);
		}//중첩되지 않았다면
		else {
			s.pop();
			if (s.size() < 1) {
				cnt += 0;
			}
			else {
				cnt += 1;
			}
			debug(10);
		}
	}
	cout << "result: " << cnt;
}
